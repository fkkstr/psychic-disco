...
/**
 * Função para buscar os dados da playlist do Spotify através do servidor proxy.
 *
 * @returns {Promise<Array>} Retorna uma Promise com a lista de itens da playlist.
 */
export async function fetchSpotifyPlaylist() {
  const url = '/api_playlist.php'; // Caminho para o seu arquivo PHP no servidor
...
/**
 * Renderiza a playlist do Spotify no DOM.
 *
 * @param {HTMLElement} container O elemento DOM onde a playlist será renderizada.
 */
export async function renderSpotifyPlaylist(container) {
  if (!container) return;
  container.innerHTML = '';
  container.classList.add('loading'); // Adiciona uma classe de carregamento
